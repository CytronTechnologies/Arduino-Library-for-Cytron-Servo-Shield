...
/*
  Example code to test Shield Servo
  ...
  Rotate servo motor continuosly
...
void setup()
{
  ...
  on_off_motor(0, 1); //activate all servo motor channels on Shield servo
}
...
void loop()
{
  ...
  { //In position reporting operation, setting speed = 0 will cause the servo motor oscillates due to limitation of software position feedback
    set_ch_pos_spd(count + 1, 6000, 50); //set position = 7000, speed = 50
  }
  digitalWrite(led_red, HIGH); //led is on (indicaton)
  while (rd_current_pos(1) < 5000); //wait untill the (position-100) is reached   (Optional)
  delay(1000);
  ...
  { //In position reporting operation, setting speed = 0 will cause the servo motor oscillates due to limitation of software position feedback
    set_ch_pos_spd(count + 1, 1000, 50); //set position = 4000, speed = 50
  }

  digitalWrite(led_red, LOW); //led is off (indicaton)
  while (rd_current_pos(1) > 2000); //wait untill the (position + 100) is reached   (Optional)
  delay(1000);
}
...
void initial_position(unsigned char channel, unsigned int position) //optional, if used, the RX pin of Arduino Mainboard should be connected to TX pin of Shield servo
{
